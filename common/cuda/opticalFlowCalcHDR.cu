...
void OpticalFlowCalcHDR::blurFrameArray(const unsigned short* frame, unsigned short* blurredFrame, const unsigned char kernelSize, const bool directOutput) {
	...
	// Scale the frame according to the renderer
	if (directOutput) {
		scaleFrameKernelHDR << <m_grid8x8x1, m_threads8x8x2 >> > (blurredFrame, m_outputFrame.arrayPtrGPU, m_iDimY, m_iDimX, m_iScaledDimY, m_iScaledDimX, m_iChannelIdxOffset, m_iScaledChannelIdxOffset);
	}
}
...
*/
void OpticalFlowCalcHDR::warpFrames(float fScalar, const int outputMode) {
...
*/
void OpticalFlowCalcHDR::insertFrame() {
	insertFrameKernel << <m_grid16x16x1, m_threads16x16x2 >> >(m_frame1.arrayPtrGPU,
