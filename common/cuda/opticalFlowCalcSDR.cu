...
__global__ void warpFrameKernelForOutputSDR(const unsigned char* frame1, const int* offsetArray, int* hitCount, int* ones,
	...
	// U/V-Channel
		...
		if (newCy >= 0 && newCy < (dimY >> 1) && newCx >= 0 && newCx < dimX) {
			// U-Channel
			if ((cx & 1) == 0) {
				warpedFrame[scaledChannelIdxOffset + newCy * scaledDimX + (newCx & ~1)] = static_cast<unsigned short>(frame1[channelIdxOffset + cy * dimX + cx]) << 8;

			// V-Channel
			} else {
				warpedFrame[scaledChannelIdxOffset + newCy * scaledDimX + (newCx & ~1) + 1] = static_cast<unsigned short>(frame1[channelIdxOffset + cy * dimX + cx]) << 8;
			}
		}
	}
}
...
__global__ void warpFrameKernelForBlendingSDR(const unsigned char* frame1, const int* offsetArray, int* hitCount, int* ones,
											  ...
											  const int dimY, const int dimX, const float resolutionDivider,
	...
	// U/V-Channel
		...
		if (newCy >= 0 && newCy < (dimY >> 1) && newCx >= 0 && newCx < dimX) {
			// U-Channel
			if ((cx & 1) == 0) {
				warpedFrame[channelIdxOffset + newCy * dimX + (newCx & ~1)] = frame1[dimY * dimX + cy * dimX + cx];

			// V-Channel
			} else {
				warpedFrame[channelIdxOffset + newCy * dimX + (newCx & ~1) + 1] = frame1[dimY * dimX + cy * dimX + cx];
			}
		}
	}
}
...
__global__ void artifactRemovalKernelForOutputSDR(const unsigned char* frame1, const int* hitCount, unsigned short* warpedFrame,
	...
	// U/V Channels
		...
		if (hitCount[threadIndex2D] != 1) {
			warpedFrame[scaledChannelIdxOffset + cy * scaledDimX + cx] = static_cast<unsigned short>(frame1[channelIdxOffset + threadIndex2D]) << 8;
		}
	}
}
...
void OpticalFlowCalcSDR::warpFramesForOutput(float fScalar, const bool bOutput12) {
	...
	if (m_bBisNewest) {
		// Frame 1 to Frame 2
		if (bOutput12) {
			warpFrameKernelForOutputSDR << <m_grid16x16x1, m_threads16x16x2 >> > (m_frame1.arrayPtrGPU, m_blurredOffsetArray12.arrayPtrGPU,
			...
			artifactRemovalKernelForOutputSDR << <m_grid8x8x1, m_threads8x8x2 >> > (m_frame1.arrayPtrGPU, m_hitCount12.arrayPtrGPU,
																	    m_outputFrame.arrayPtrGPU, m_iDimY, m_iDimX, m_iScaledDimX, m_iChannelIdxOffset, m_iScaledChannelIdxOffset);
		// Frame 2 to Frame 1
		} else {
			warpFrameKernelForOutputSDR << <m_grid16x16x1, m_threads16x16x2 >> > (m_frame2.arrayPtrGPU, m_blurredOffsetArray21.arrayPtrGPU,
																  m_hitCount21.arrayPtrGPU, m_ones.arrayPtrGPU, 
																  m_outputFrame.arrayPtrGPU, frameScalar21, m_iLowDimY, m_iLowDimX,
																  m_iDimY, m_iDimX, m_fResolutionDivider, m_iLayerIdxOffset, m_iScaledDimX, m_iChannelIdxOffset, m_iScaledChannelIdxOffset);
			artifactRemovalKernelForOutputSDR << <m_grid8x8x1, m_threads8x8x2 >> > (m_frame2.arrayPtrGPU, m_hitCount21.arrayPtrGPU,
																	    m_outputFrame.arrayPtrGPU, m_iDimY, m_iDimX, m_iScaledDimX, m_iChannelIdxOffset, m_iScaledChannelIdxOffset);
		}
	} else {
		// Frame 1 to Frame 2
		if (bOutput12) {
			warpFrameKernelForOutputSDR << <m_grid16x16x1, m_threads16x16x2 >> > (m_frame2.arrayPtrGPU, m_blurredOffsetArray12.arrayPtrGPU,
																  m_hitCount12.arrayPtrGPU, m_ones.arrayPtrGPU,
																  m_outputFrame.arrayPtrGPU, frameScalar12, m_iLowDimY, m_iLowDimX,
																  m_iDimY, m_iDimX, m_fResolutionDivider, m_iLayerIdxOffset, m_iScaledDimX, m_iChannelIdxOffset, m_iScaledChannelIdxOffset);
			artifactRemovalKernelForOutputSDR << <m_grid8x8x1, m_threads8x8x2 >> > (m_frame2.arrayPtrGPU, m_hitCount12.arrayPtrGPU,
																		m_outputFrame.arrayPtrGPU, m_iDimY, m_iDimX, m_iScaledDimX, m_iChannelIdxOffset, m_iScaledChannelIdxOffset);
		// Frame 2 to Frame 1
		} else {
			warpFrameKernelForOutputSDR << <m_grid16x16x1, m_threads16x16x2 >> > (m_frame1.arrayPtrGPU, m_blurredOffsetArray21.arrayPtrGPU,
																  m_hitCount21.arrayPtrGPU, m_ones.arrayPtrGPU,
																  m_outputFrame.arrayPtrGPU, frameScalar21, m_iLowDimY, m_iLowDimX,
																  m_iDimY, m_iDimX, m_fResolutionDivider, m_iLayerIdxOffset, m_iScaledDimX, m_iChannelIdxOffset, m_iScaledChannelIdxOffset);
			artifactRemovalKernelForOutputSDR << <m_grid8x8x1, m_threads8x8x2 >> > (m_frame1.arrayPtrGPU, m_hitCount21.arrayPtrGPU,
																	    m_outputFrame.arrayPtrGPU, m_iDimY, m_iDimX, m_iScaledDimX, m_iChannelIdxOffset, m_iScaledChannelIdxOffset);
		}
	}
}
...
void OpticalFlowCalcSDR::warpFramesForBlending(float fScalar) {
	...
	if (m_bBisNewest) {
		// Frame 1 to Frame 2
		warpFrameKernelForBlendingSDR << <m_grid16x16x1, m_threads16x16x2, 0, warpStream1 >> > (m_frame1.arrayPtrGPU, m_blurredOffsetArray12.arrayPtrGPU,
														          m_hitCount12.arrayPtrGPU, m_ones.arrayPtrGPU, 
		...
		// Frame 2 to Frame 1
		warpFrameKernelForBlendingSDR << <m_grid16x16x1, m_threads16x16x2, 0, warpStream2 >> > (m_frame2.arrayPtrGPU, m_blurredOffsetArray21.arrayPtrGPU,
																  m_hitCount21.arrayPtrGPU, m_ones.arrayPtrGPU, 
		...
		// Frame 1 to Frame 2
		warpFrameKernelForBlendingSDR << <m_grid16x16x1, m_threads16x16x2, 0, warpStream1 >> > (m_frame2.arrayPtrGPU, m_blurredOffsetArray12.arrayPtrGPU,
																  m_hitCount12.arrayPtrGPU, m_ones.arrayPtrGPU,
																  m_warpedFrame12.arrayPtrGPU, frameScalar12, m_iLowDimY, m_iLowDimX,
																  m_iDimY, m_iDimX, m_fResolutionDivider, m_iLayerIdxOffset, m_iChannelIdxOffset);
		...
		// Frame 2 to Frame 1
		warpFrameKernelForBlendingSDR << <m_grid16x16x1, m_threads16x16x2, 0, warpStream2 >> > (m_frame1.arrayPtrGPU, m_blurredOffsetArray21.arrayPtrGPU,
																  m_hitCount21.arrayPtrGPU, m_ones.arrayPtrGPU,
																  m_warpedFrame21.arrayPtrGPU, frameScalar21, m_iLowDimY, m_iLowDimX,
																  m_iDimY, m_iDimX, m_fResolutionDivider, m_iLayerIdxOffset, m_iChannelIdxOffset);
...
void OpticalFlowCalcSDR::blendFrames(float fScalar) {
	...
	// Blend the frames
	blendFrameKernelSDR << <m_grid16x16x1, m_threads16x16x2 >> >(m_warpedFrame12.arrayPtrGPU, m_warpedFrame21.arrayPtrGPU,
...
void OpticalFlowCalcSDR::drawFlowAsHSV(const float blendScalar) const {
	if (m_bBisNewest) {
		convertFlowToHSVKernelSDR << <m_grid16x16x1, m_threads16x16x2 >> > (m_blurredOffsetArray12.arrayPtrGPU, m_outputFrame.arrayPtrGPU,
