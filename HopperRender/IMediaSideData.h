...
// -----------------------------------------------------------------
// This interface should be implemented in IMediaSample objects and accessed
// through IUnknown It allows binary side data to be attached to the media
// samples and delivered with them Restrictions: Only one side data per type can
// be attached
interface __declspec(uuid(
    "F940AE7F-48EB-4377-806C-8FC48CAB2292")) IMediaSideData : public IUnknown {
    ...
    // The caller receives pointers to the internal data, and the pointers shall
    // stay valid for the lifetime of the object
    STDMETHOD(GetSideData)(GUID guidType, const BYTE** pData, size_t* pSize)
	PURE;
};
...
DEFINE_GUID(IID_MediaSideDataHDR, 0x53820dbc, 0xa7b8, 0x49c4, 0xb1, 0x7b, 0xe5,
	    0x11, 0x59, 0x1a, 0x79, 0xc);
...
DEFINE_GUID(IID_MediaSideDataHDRContentLightLevel, 0xed6ae576, 0x7cbe, 0x41a6,
	    0x9d, 0xc3, 0x7, 0xc3, 0x5d, 0xc1, 0x3e, 0xf9);
...
DEFINE_GUID(IID_MediaSideDataHDR10Plus, 0x183ed511, 0x8910, 0x4262, 0x88, 0xf6,
	    0x49, 0x46, 0xbc, 0x79, 0x9c, 0x84);

#pragma pack(push, 1)
// HDR10+ metadata according to SMPTE 2094-40
// Refer to the specification for the meaning of the fields
//
// All pixel values are kept as-is, rational values are normalized as
...
DEFINE_GUID(IID_MediaSideDataDOVIRPU, 0xbae40e6c, 0x5b93, 0x4170, 0x90, 0xcc,
	    0x5d, 0x5f, 0x2, 0xa2, 0x96, 0x38);

// {277EE779-13F4-434E-BDEC-3D6F8C0E15D2}
DEFINE_GUID(IID_MediaSideDataDOVIMetadata, 0x277ee779, 0x13f4, 0x434e, 0xbd,
	    0xec, 0x3d, 0x6f, 0x8c, 0xe, 0x15, 0xd2);

// {F1949F1C-7474-4F0F-B110-FB734C08851D}
DEFINE_GUID(IID_MediaSideDataDOVIMetadataV2, 0xf1949f1c, 0x7474, 0x4f0f, 0xb1,
	    0x10, 0xfb, 0x73, 0x4c, 0x8, 0x85, 0x1d);
...
#define LAV_DOVI_MAX_PIECES 8
	 ...
	 * be used instead of the matrices indicated by the frame's colorspace
	 * tags. The output of rgb_to_lms_matrix is to be fed into a BT.2020
	 * LMS->RGB matrix based on a Hunt-Pointer-Estevez transform, but
	 * without any crosstalk. (See the definition of the ICtCp colorspace
...
DEFINE_GUID(IID_MediaSideData3DOffset, 0xf169b76c, 0x75a3, 0x49e6, 0xa2, 0x3a,
	    0x14, 0x98, 0x3e, 0xbf, 0x43, 0x70);
...
struct MediaSideData3DOffset {
    ...
    // positive values offset closer to the viewer (move right on the left view,
    // left on the right view) negative values offset further away from the
    // viewer (move left on the left view, right on the right view)
    int offset[32];
};
#pragma pack(pop)
...
DEFINE_GUID(IID_MediaSideDataEIA608CC, 0x40fefd7f, 0x85dd, 0x4335, 0xa8, 0x4,
	    0x8a, 0x33, 0xb0, 0xbf, 0x7b, 0x81);

// There is no struct definition. The data is supplied as a list of 3 byte CC
// data packets (control byte + cc_data1/2)
...
DEFINE_GUID(IID_MediaSideDataControlFlags, 0xb5411ce3, 0x3b21, 0x4fd6, 0x80,
	    0xa8, 0xce, 0x68, 0x29, 0x69, 0xf7, 0x95);
