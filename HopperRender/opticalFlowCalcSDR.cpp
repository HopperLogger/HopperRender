...
void OpticalFlowCalcSDR::updateFrame(unsigned char* inputPlanes) {
    CHECK_ERROR(clEnqueueWriteBuffer(m_queue, m_inputFrameArray[0], CL_TRUE, 0, m_frameHeight * m_actualWidth + (m_frameHeight / 2) * m_actualWidth, inputPlanes, 0, NULL, &m_ofcStartedEvent));
...
OpticalFlowCalcSDR::OpticalFlowCalcSDR(const int frameHeight,
				       ...
				       float whiteLevel, int maxCalcRes) {
    // Set up variables
    ...
    m_inputFrameArray[0] = clCreateBuffer(m_clContext, CL_MEM_READ_ONLY, 1.5 * m_frameHeight * m_actualWidth, NULL, &err);
    m_inputFrameArray[1] = clCreateBuffer(m_clContext, CL_MEM_READ_ONLY, 1.5 * m_frameHeight * m_actualWidth, NULL, &err);
