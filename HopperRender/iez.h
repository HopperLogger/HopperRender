...
DECLARE_INTERFACE_(SettingsInterface, IUnknown) {
	STDMETHOD(GetCurrentSettings)(THIS_
		...
		int* piFrameRateMode, // How the target FPS is determined (0: Display refresh rate, 1: Custom target FPS, >=2: integer multiplier of the source FPS)
		int* piDeltaScalar,
	...
	STDMETHOD(UpdateUserSettings)(THIS_
		...
		int iFrameRateMode, // How the target FPS is determined (0: Display refresh rate, 1: Custom target FPS, >=2: integer multiplier of the source FPS)
		int iDeltaScalar,
		int iNeighborScalar,
		int iBlackLevel,
		int iWhiteLevel,
