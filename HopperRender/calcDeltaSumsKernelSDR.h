#pragma once
const char* calcDeltaSumsKernelSDR = R"CLC(
...
__kernel void calcDeltaSumsKernel(__global unsigned int* summedUpDeltaArray, __global const unsigned char* frame1,
    ...
    if (cy >= lowDimY || cx >= lowDimX) {
        ...
        if (scaledCx < 0 || scaledCx >= dimX || scaledCy < 0 || scaledCy >= dimY) {
            ...
            delta = abs_diff(frame1[newCy * inputStride + newCx], frame2[scaledCy * inputStride + scaledCx]) + 
                    abs_diff(frame1[dimY * inputStride + (newCy >> 1) * inputStride + (newCx & ~1)], frame2[dimY * inputStride + (scaledCy >> 1) * inputStride + (scaledCx & ~1)]) + 
                    abs_diff(frame1[dimY * inputStride + (newCy >> 1) * inputStride + (newCx & ~1) + 1], frame2[dimY * inputStride + (scaledCy >> 1) * inputStride + (scaledCx & ~1) + 1]);
            delta <<= deltaScalar;
        }
