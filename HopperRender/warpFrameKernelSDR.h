...
__kernel void warpFrameKernel(__global const unsigned char* sourceFrame12, __global const unsigned char* sourceFrame21,
    ...
    if (frameOutputMode == 5 && cx < (actualDimX >> 1)) {
        outputFrame[cz * dimY * dimX + cy * dimX + cx] = sourceFrame12[cz * dimY * actualDimX + cy * actualDimX + cx] << 8;
        return;
        ...
        if (isInLeftSide) { // Place the source frame in the left side of the output frame
            outputFrame[cz * dimY * dimX + cy * dimX + cx] = sourceFrame12[cz * dimY * actualDimX + ((cy - (verticalOffset >> cz)) << 1) * actualDimX + (cx << 1) + (cz ? (cx & 1) : 0)] << 8;
            return;
    ...
    if (frameOutputMode == 0) { // WarpedFrame12
        outputFrame[cz * dimY * dimX + cy * dimX + cx] = sourceFrame12[cz * dimY * actualDimX + newCy12 * actualDimX + (newCx12 & (cz ? ~1 : ~0)) + (cx & (cz ? 1 : 0))] << 8;
    } else if (frameOutputMode == 1) { // WarpedFrame21
        outputFrame[cz * dimY * dimX + cy * dimX + cx] = sourceFrame21[cz * dimY * actualDimX + newCy21 * actualDimX + (newCx21 & (cz ? ~1 : ~0)) + (cx & (cz ? 1 : 0))] << 8;
    } else { // BlendedFrame
