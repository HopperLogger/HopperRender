...
#include "adjustOffsetArrayKernelHDR.h"
#include "blurFlowKernelHDR.h"
#include "calcDeltaSumsKernelHDR.h"
#include "determineLowestLayerKernelHDR.h"
#include "warpFrameKernelHDR.h"
...
void OpticalFlowCalcHDR::updateFrame(unsigned char* inputPlanes) {
    ...
    m_inputFrameArray[1] = m_inputFrameArray[2];
    m_inputFrameArray[2] = temp0;
...
void OpticalFlowCalcHDR::calculateOpticalFlow() {
    ...
    for (int iter = 0; iter < opticalFlowIterations; iter++) {
        for (int step = 0; step < 2; step++) {
            ...
            cl_int err = clSetKernelArg(m_calcDeltaSumsKernel, 1, sizeof(cl_mem), &m_inputFrameArray[1]);
            err |= clSetKernelArg(m_calcDeltaSumsKernel, 2, sizeof(cl_mem), &m_inputFrameArray[2]);
    ...
    CHECK_ERROR(clSetKernelArg(m_blurFlowKernel, 1, sizeof(cl_mem), &m_blurredOffsetArray[0]));
    ...
    cl_mem temp0 = m_blurredOffsetArray[0];
    m_blurredOffsetArray[0] = m_blurredOffsetArray[1];
    m_blurredOffsetArray[1] = temp0;
    cl_ulong start_time, end_time;
...
void OpticalFlowCalcHDR::warpFrames(const float blendingScalar, const int frameOutputMode) {
    ...
    err |= clSetKernelArg(m_warpFrameKernel, 2, sizeof(cl_mem), &m_blurredOffsetArray[0]);
...
void OpticalFlowCalcHDR::copyFrame() {
    ...
    // Copy Frame
    int cz = 0; // Y-Plane
	int frameIndex = m_frameCount >= 3 ? 0 : m_frameCount >= 2 ? 1 : 2;
...
OpticalFlowCalcHDR::~OpticalFlowCalcHDR() {
    clFinish(m_queue);
    clReleaseMemObject(m_inputFrameArray[0]);
    clReleaseMemObject(m_inputFrameArray[1]);
    clReleaseMemObject(m_inputFrameArray[2]);
    clReleaseMemObject(m_outputFrameArray);
    clReleaseMemObject(m_offsetArray);
    clReleaseMemObject(m_blurredOffsetArray[0]);
    clReleaseMemObject(m_blurredOffsetArray[1]);
...
OpticalFlowCalcHDR::OpticalFlowCalcHDR(const int frameHeight, const int frameWidth,
    ...
    m_inputFrameArray[2] = clCreateBuffer(m_clContext, CL_MEM_READ_ONLY, 3 * m_frameHeight * m_inputStride, NULL, &err);
    ...
    m_blurredOffsetArray[0] = clCreateBuffer(m_clContext, CL_MEM_READ_WRITE, 2 * m_opticalFlowFrameHeight * m_opticalFlowFrameWidth * sizeof(short), NULL, &err);
    m_blurredOffsetArray[1] = clCreateBuffer(m_clContext, CL_MEM_READ_WRITE, 2 * m_opticalFlowFrameHeight * m_opticalFlowFrameWidth * sizeof(short), NULL, &err);
    ...
    // Compile the kernels
    cl_create_kernel(&m_calcDeltaSumsKernel, m_clContext, m_clDeviceId, "calcDeltaSumsKernel", calcDeltaSumsKernelHDR);
    cl_create_kernel(&m_determineLowestLayerKernel, m_clContext, m_clDeviceId, "determineLowestLayerKernel", determineLowestLayerKernelHDR);
    cl_create_kernel(&m_adjustOffsetArrayKernel, m_clContext, m_clDeviceId, "adjustOffsetArrayKernel", adjustOffsetArrayKernelHDR);
    cl_create_kernel(&m_blurFlowKernel, m_clContext, m_clDeviceId, "blurFlowKernel", blurFlowKernelHDR);
    cl_create_kernel(&m_warpFrameKernel, m_clContext, m_clDeviceId, "warpFrameKernel", warpFrameKernelHDR);
    cl_create_kernel(&m_copyFrameKernel, m_clContext, m_clDeviceId, "copyFrameKernel", copyFrameKernelHDR);
